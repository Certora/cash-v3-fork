...
methods {
  ...
  function _.isAdmin(address) external => DISPATCHER(true);
  function _.proxiableUUID() external with (env e) => cvlProxiableUUID(e) expect bytes32;
  function _.useNonce() external => DISPATCHER(true);

  // Moved from sanity_.*.spec
...
// Define ghost variables to store the constant return values
ghost bool constant_bool_result;
ghost uint256 constant_uint;

function nondetVerifySignature() {
  bool b;
  require b;
}
