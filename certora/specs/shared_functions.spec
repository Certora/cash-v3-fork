methods {
    // function RoleRegistry.hasRole(address holder, uint256 role) internal returns (bool) => cvlHasRole(holder, role);
    // function EnumerableRoles._setRole(address holder, uint256 role, bool active) internal => cvlSetRole(holder, role, active);
    // function RoleRegistry.roleHolders(uint256 role) internal returns (address[] memory) => cvlRoleHolders(role);
    // function RoleRegistry.configureSafeAdmins(address[], bool[]) external => cvlConfigureSafeAdmins();
}
